...
trueValsNodes=SinCos(Points[:,0],Points[:,1])
...
uApprox_on_Grid=P1FEM_periodicConditions.evalFEM_periodicPipe_unifGrid(coordinatesGrid,u,Nx,Ny,Lx,Ly)
...
#               Second function
################################################
def complexPatternFunc(X,Y) :
    Z = np.cos(2*np.pi*X)*np.sin(2*np.pi*Y)+0.1*np.sin(2*np.pi*13*X)*np.sin(2*np.pi*17*Y)*np.exp(3*np.sin(2*np.pi*Y))
    Z = 0.3*Z
    return Z
